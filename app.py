...
# Код переписан. Добавлен в функцию аргумент
...
def show_all_docs_info():
    tmp_list = []
    for current_document in documents:
        tmp_list.append(show_document_info(current_document))
    return tmp_list
...
def secretary_program_start():
    ...
    while True:
        ...
        if user_command == 'p':
            ...
            new_doc = add_new_doc(new_doc_number, new_doc_type, new_doc_owner_name, new_doc_shelf_number)
            print('\nНа полку "{}" добавлен новый документ:'.format(new_doc))
        elif user_command == 'd':
            doc_num = input('Документ, который необходимо удалить - ')
            doc_number, deleted = delete_doc(doc_num)
            if deleted:
